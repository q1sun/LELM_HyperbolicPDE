...
## parser arguments
parser = argparse.ArgumentParser(description='Lift-and-Learning Methods for Solving Buckley-Leverett Equation')
# weights
...
parser.add_argument('--inits', type=float, default=1.3, help='The initial guess of shock position x(0)') 
...
# datasets options
parser.add_argument('--num_intrr_pts', type=int, default=20000, help='total number of sampling points at interior domain')
...
class TraindataShock(Dataset):
    def __init__(self, num_shock_pts, s, dim_prob):
        self.SmpPts_Shock = Sample_Point.SmpPts_Shock(num_shock_pts, s, dim_prob)
...
def fprime(S, M=M):
    # return analytical derivative f'(S) (works elementwise for torch tensors)
    D = S**2 + M * (1.0 - S)**2
    return (2.0 * M * S * (1.0 - S)) / (D**2)
...
def train_epoch(epoch, model, optimizer, device):
    ...
    # copy the value of s
    shock_speed = shock_param.detach()
    # creating shock line by the value of shock speed
    traindata_shock = TraindataShock(args.num_shock_pts, shock_speed, dim_prob)
    ...
    for i, (data_intrr, data_bndry, data_initl, data_shock) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry), cycle(dataloader_initl), cycle(dataloader_shock))):
        ...
        f_R_minus_F_L = f_flux(u_NN_shockr) - f_flux(u_NN_shockl)
        # construct mini-batch loss function and then perform backward pass
        loss_intrr = torch.mean(torch.pow(gradu_NN_intrr[:,1] + torch.squeeze(fprime(u_NN_intrr)) * gradu_NN_intrr[:,0], 2))
        loss_bndry = torch.mean(torch.pow(u_NN_bndrL - 1, 2))
        loss_initl = torch.mean(torch.pow(torch.squeeze(u_NN_initl) - u0val_smppts, 2))
        loss_shock = torch.mean(torch.pow(torch.squeeze(u_NN_shockl) * fprime(torch.squeeze(u_NN_shockl)) - f_flux(torch.squeeze(u_NN_shockl)), 2)) + torch.mean(torch.pow(shock_param.to(device) - fprime(u_NN_shockl), 2))
        loss_minibatch = loss_intrr + loss_bndry + 10 * loss_shock + args.beta * (loss_initl)
    ...
    return loss_intrr_epoch, loss_bndry_epoch, loss_initl_epoch, loss_shock_epoch, loss_epoch
...
shock_param = torch.nn.Parameter(args.inits * torch.ones(1, requires_grad=True))
hist_shock_speed = []

param_groups = [
    {"params": model.parameters(), "lr": 0.01},
    {"params": [shock_param], "lr": 0.001},
]
...
for epoch in range(args.num_epochs):
    ...
    trainloss_intrr_epoch, trainloss_bndry_epoch, trainloss_initl_epoch, trainloss_shock_epoch, trainloss_epoch = train_epoch(epoch, model, optimizer, device)
    testloss_u_epoch = 0
...
with torch.no_grad():  
    temp = torch.ones(1001, 1) * 0.4
    init = torch.zeros(1001, 1)
    test_smppts = torch.cat([torch.linspace(0, 1, steps=1001).reshape(-1, 1) * 2 - 1, temp.reshape(-1,1)], dim=1)
    test_smppts = torch.cat([test_smppts, Exact_Solution.augmented_variable(test_smppts[:,0], test_smppts[:,1], shock_param).reshape(-1,1)], dim=1)
    init_smppts = torch.cat([torch.linspace(0, 1, steps=1001).reshape(-1, 1) * 2 - 1, init.reshape(-1,1)], dim=1)
    init_smppts = torch.cat([init_smppts, Exact_Solution.augmented_variable(init_smppts[:,0], init_smppts[:,1], shock_param).reshape(-1,1)], dim=1)
...
fig=plt.figure()
u_Exact = Exact_Solution.u_Exact_Solution(test_smppts[:,0].cpu(), test_smppts[:,1].cpu(), M).detach().numpy().reshape(1001, 1)
plt.plot(x, u_Exact, ls = '-', lw = '2')
plt.title('Exact Solution u on Test Dataset')
#plt.show()  
fig.savefig(os.path.join(args.figures,'Exact_u_testdata_N.png'))
plt.close(fig)
...
fig = plt.figure()
u_init = u_init.cpu().detach().numpy().reshape(1001, 1)
...
plt.hlines(fprime(np.sqrt(M/(M+1))), 0, len(hist_shock_speed), label=r"s_ture", color="tab:green")
plt.legend()
...
fig = plt.figure()
plt.title(r"s")
plt.plot(np.log10(abs(np.array(hist_shock_speed) - fprime(np.sqrt(M/(M+1))))), label=r"s_error")
plt.legend()
plt.xlabel("Training step")
fig.savefig(os.path.join(args.figures, 'Predited_s_err'))
plt.show()
