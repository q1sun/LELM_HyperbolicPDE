...
##################################################################################################
## parser arguments
parser = argparse.ArgumentParser(description='Lift-and-Learning Methods for Solving Linear Convection Equation in Section 4-1-2')
# weights
...
# hyperparameters
parser.add_argument('--num_epochs', default=25000, type=int, metavar='N', help='total number of training epochs')
parser.add_argument('--beta', default=400, type=int, metavar='N', help='penalty coefficeint')
parser.add_argument('--milestones', type=int, nargs='+', default=[8000, 15000, 20000, 24000], help='decay of learning rate at these epochs')
...
# datasets options
parser.add_argument('--num_intrr_pts', type=int, default=40000, help='total number of sampling points at interior domain')
parser.add_argument('--num_initl_pts', type=int, default=5000, help='total number of sampling points for initial condition')
parser.add_argument('--num_bndry_pts', type=int, default=5000, help='total number of sampling points for boundary condition')
...
def train_epoch(epoch, model, optimizer, device):
    ...
    for i, (data_intrr, data_bndry, data_initl, data_shock) in enumerate(zip(dataloader_intrr, cycle(dataloader_bndry), cycle(dataloader_initl), cycle(dataloader_shock))):
        ...
        loss_intrr = torch.mean(torch.pow(a * gradu_NN_intrr[:,0] + gradu_NN_intrr[:,1], 2))
        loss_bndry = torch.mean(torch.pow(u_NN_bndrL - u_NN_bndrR, 2))
        loss_initl = torch.mean(torch.pow(torch.squeeze(u_NN_initl) - u0val_smppts, 2))

        loss_shock1 = torch.mean(torch.pow(u_NN_shk1R - u_NN_shk1L - 1, 2))
        loss_shock2 = torch.mean(torch.pow(u_NN_shk2R - u_NN_shk2L + 1, 2))
...
with torch.no_grad():  
    ...
    u_smppts = torch.cat([torch.linspace(0, 1, 1001).reshape(-1, 1) * (x_max - x_min) + x_min, 0.5 * torch.ones(1001, 1).reshape(-1,1)], dim=1)
